...
def summarize(det_results, annotations, num_classes, thresholds: Sequence[float] = (0.5, 0.75), mode='area') -> Dict[str, float]:
    ...
    r = eval_rbbox_map(det_results, annotations, num_classes, 0.5, mode)
    res['recall@0.50'] = float(np.mean([r[f'recall{c}'] for c in range(num_classes)]))
    return res
