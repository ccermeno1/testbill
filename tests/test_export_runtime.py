"""
Export a ONNX y el pre/post-proceso del cliente.

Lo que se comprueba es lo que de verdad rompe un despliegue: que el grafo exportado y el
post-proceso en numpy reproduzcan las detecciones del modelo PyTorch, incluida la convencion
de esquinas (equivocarse de signo en el angulo da cajas plausibles pero mal orientadas).
"""
...
def _onnx_example():
    return pytest.importorskip("onnx_example", reason="hace falta el paquete del repo")


def test_numpy_rbox2poly_matches_torch():
    """La conversion a poligono del cliente debe coincidir con la del modelo."""
    mod = _onnx_example()
...
def test_numpy_rotated_nms_matches_torch():
    ...
    boxes = np.array(
        [
            ...
            [102.0, 101.0, 60.0, 30.0, 0.05],   # casi la misma
...
def test_onnx_export_matches_torch(tmp_path):
    """El grafo exportado debe dar las mismas detecciones que el modelo PyTorch."""
    ...
    # y el post-proceso en numpy debe coincidir con el de PyTorch sobre esas mismas salidas
