"""
Geometria de cajas rotadas y NMS.

La IoU rotada se contrasta contra Shapely, que es una implementacion independiente: es la
pieza de la que dependen el asignador, la metrica y el NMS, asi que un error aqui se propaga
a todo lo demas sin dar la cara.
"""
...
def test_poly2rbox_survives_mirroring_and_reordering():
    """La rbox derivada debe describir el MISMO rectangulo que el poligono de entrada, aunque
    venga espejado o con los vertices rotados: un flip invierte la orientacion del poligono."""
    from shapely.geometry import Polygon

    rng = np.random.default_rng(0)
    transforms = {
        "identidad": lambda p: p,
        "espejo horizontal": lambda p: (p.reshape(4, 2) * [-1, 1] + [640, 0]).reshape(8),
        "orden invertido": lambda p: p.reshape(4, 2)[::-1].reshape(8),
        "vertices rotados": lambda p: np.roll(p.reshape(4, 2), 1, 0).reshape(8),
    }
    for nombre, transform in transforms.items():
        for _ in range(50):
            ...
            assert iou > 0.99, f"{nombre}: IoU {iou:.3f} entre el poligono y su rbox"
...
def test_rotated_iou_chunking_is_transparent():
    """El troceado que acota la VRAM no debe cambiar el resultado."""
    ...
    try:
        ...
        B.MAX_PAIRS = 41          # fuerza varios trozos
