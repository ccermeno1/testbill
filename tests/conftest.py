"""Shared fixtures and synthetic dataset builders."""
...
def model():
    """PP-YOLOE-R-s with random weights, shared by the tests that only check shapes."""
    torch.manual_seed(0)
    return build_ppyoloe_r(num_classes=1, size="s").eval()


def shapely_iou(a, b) -> float:
    """Reference IoU computed with Shapely, independent of our own implementation."""
...
def make_dataset(tmp_path):
    """Noise images with one fixed box each. Returns (annotation file, image folder)."""
    import cv2
...
def make_learnable_dataset(tmp_path):
    """A bright rectangle on a dark background: synthetic but learnable."""
    import cv2
...
def write_coco(tmp_path):
    """Write a hand-made COCO file, handy for annotation edge cases."""
    import cv2
