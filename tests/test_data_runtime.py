...
Several of these pin down details found by diffing against PaddleDetection. When they were
missing, training quietly got worse without raising anything.
"""
...
def test_dataset_train_augmentation_keeps_boxes(make_dataset):
    ...
    ds.set_epoch(5)  # past the mosaic phase
    assert ds[0]["image"].ndim == 3


def test_dataset_filters_degenerate_boxes(write_coco):
    """Boxes with a side under 2 px are dropped, like ppdet's Poly2RBox. If they reach the
    loss, ProbIoU hits log(0) and training goes to NaN."""
...
def test_dataset_clips_polygons_to_canvas(write_coco):
    """Vertices are clipped to the canvas, like ppdet's RResize.

    Without the clip the model is trained on the full extent of objects that are only partly
    visible, which inflates the predicted boxes and tanks mAP75.
    """
...
def test_rotation_shrinks_instead_of_cropping(make_dataset):
    """Rotations use auto_bound, so the rotated content fits the canvas whole."""
    ...
    assert out.shape == img.shape                      # the canvas does not grow
