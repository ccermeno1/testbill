#!/usr/bin/env python
"""
Integra una carpeta de copias aumentadas offline (augmented/images + augmented/labels en formato
YOLOv8-OBB, ids `<id_origen>__augN`) en dataset/banknotes_obb y genera
annotations/train_plus_aug.json = train.json + copias aumentadas.

Comprueba que cada copia procede de una imagen del split TRAIN (por nombre base, sin hash);
si alguna viene de valid/test, aborta (seria una fuga).

  python scripts/add_augmented.py --src augmented --dataset dataset/banknotes_obb
"""
import argparse
import json
import os
...
def main():
    ...
    if not imgs:
        sys.exit(f"no hay imagenes en {args.src}/images")
    bad, n_boxes, sources = [], 0, set()
    for p in imgs:
        base = os.path.splitext(os.path.basename(p))[0]
        m = RX.match(base)
        if not m:
            sys.exit(f"nombre no reconocido (esperado <origen>__augN): {base}")
        ...
        train["images"].append({"id": img_id, "file_name": f"{stem}.jpg", "width": W, "height": H,
                                ...
                                "source": src_stem})
    if bad:
        sys.exit(f"FUGA: {len(bad)} copias proceden de imagenes que no son de train: {bad[:5]}")

    out = os.path.join(ann_dir, f"{args.out_name}.json")
    with open(out, "w", encoding="utf-8") as f:
        json.dump(train, f, indent=1)
    print(f"train original: {n_src_train} imgs / {n_src_ann} cajas")
    print(f"aumentadas:     {len(imgs)} imgs / {n_boxes} cajas, procedentes de {len(sources)} imagenes de train")
    print(f"total:          {len(train['images'])} imgs / {len(train['annotations'])} cajas -> {out}")
