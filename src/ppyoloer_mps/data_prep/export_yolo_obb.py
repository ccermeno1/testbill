#!/usr/bin/env python
"""
Export a converted dataset (images + labelTxt + classes.txt) back to YOLOv8-OBB format with
normalised coordinates, ready to upload to Roboflow or feed to Ultralytics.

  python export_yolo_obb.py --src data/eurobanknotes_extra --out export/eurobanknotes_yolov8obb

Output: <out>/train/images/*.jpg, <out>/train/labels/*.txt, <out>/data.yaml
"""
...
def main():
    ...
    ap.add_argument("--split", default="train", help="split folder name in the output")
    ap.add_argument("--clip", action="store_true", help="clip coordinates to [0,1]")
    args = ap.parse_args()
    ...
    print(f"{n_img} images, {n_box} boxes -> {os.path.abspath(args.out)}")
