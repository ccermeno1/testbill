#!/usr/bin/env python
"""
Dibuja ground truth (rojo) y predicciones (verde, con score) sobre las imagenes de un dataset
convertido (dataset/<x>/images + labelTxt) usando el predictions.json de predict_obb.py.

  python scripts/visualize_obb.py --dataset_dir dataset/billetesprueba \
      --predictions output/predictions_billetesprueba/predictions.json \
      --output_dir output/predictions_billetesprueba/vis_gt_pred

Salida: <output_dir>/<stem>.jpg (lado maximo --max_side px) y hojas de contacto
<output_dir>/_sheet_NN.jpg con --cols x --rows miniaturas, ordenadas de peor a mejor IoU.
"""
import argparse
import json
import os
...
def worst_iou(gts, preds):
    """IoU max alcanzado por cada GT; devuelve el minimo (1.0 si no hay GT, 0 si hay GT sin pred)."""
    if not gts:
...
def main():
    ...
    ap.add_argument("--predictions", required=True, help="predictions.json de predict_obb.py")
    ...
    # hojas de contacto, de peor a mejor
    ...
    print("rojo = ground truth, verde = prediccion (score). Hojas ordenadas de peor a mejor IoU.")
