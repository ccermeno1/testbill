"""
Metricas de deteccion orientada: AP estilo COCO (101 puntos) con IoU rotada exacta,
mAP@0.5, mAP@0.75, mAP@[0.50:0.95] y precision/recall/F1 a un umbral de confianza.

Reproduce lo que hacia `scripts/evaluate_obb.py` en la version con Paddle, para que los
numeros sean comparables entre ambas implementaciones.
"""
...
def _match(scores: np.ndarray, ious: np.ndarray, thr: float) -> np.ndarray:
    """Emparejamiento codicioso estilo COCO: por score descendente al GT libre de mayor IoU."""
...
def evaluate_detections(
    ...
    """predictions / ground_truth: listas alineadas de dicts con 'rboxes' (N, 5) y, en las
    predicciones, 'scores' (N,). Tensores o arrays; se evaluan en CPU."""
