"""
Geometria de cajas rotadas (OBB) en PyTorch puro. Funciona en CPU, CUDA y MPS.

Convenciones (las mismas que PP-YOLOE-R en PaddleDetection):
  rbox  = (cx, cy, w, h, angle)  angulo en RADIANES, sentido horario en imagen (y hacia abajo)
  poly  = (x1, y1, x2, y2, x3, y3, x4, y4)  cuatro esquinas consecutivas
"""
...
def box2corners(boxes: torch.Tensor) -> torch.Tensor:
    """(..., 5) -> (..., 4, 2). Equivalente a `box2corners` de ppdet/modeling/rbox_utils.py."""
    ...
    # matriz de rotacion traspuesta, aplicada por la derecha: [[cos, sin], [-sin, cos]]
    rot_t = torch.stack(
...
def poly2rbox(polys: torch.Tensor) -> torch.Tensor:
    """(..., 8) -> (..., 5) con angulo en [0, pi/2), como el `Poly2RBox` con rbox_type='oc'.

    Usa el lado 0->1 como referencia y normaliza a la convencion "opencv": el angulo se
    lleva al primer cuadrante intercambiando w y h cuando hace falta.
    """
    ...
    half_pi = math.pi / 2
    # llevar el angulo a [0, pi/2) intercambiando w/h segun el cuadrante
...
def check_points_in_rotated_boxes(points: torch.Tensor, boxes: torch.Tensor) -> torch.Tensor:
    """points (1, L, 2), boxes (B, N, 5) -> (B, N, L) booleano."""
...
# --------------------------------------------------------------------------------------
# ProbIoU (https://arxiv.org/abs/2106.06072), identico a ppdet/modeling/losses/probiou_loss.py
# --------------------------------------------------------------------------------------
...
def probiou(pred: torch.Tensor, target: torch.Tensor, eps: float = 1e-3) -> torch.Tensor:
    """Distancia ProbIoU (modo 'l1') entre pares de rboxes: (N, 5) x (N, 5) -> (N,)."""
...
# --------------------------------------------------------------------------------------
# IoU rotada exacta (interseccion de dos rectangulos convexos), vectorizada.
# Mismo algoritmo que la op custom `rbox_iou` de PaddleDetection (adaptada de detectron2):
# candidatos = intersecciones arista-arista + vertices interiores; orden angular; shoelace.
# --------------------------------------------------------------------------------------
def _edge_intersections(c1: torch.Tensor, c2: torch.Tensor):
    """c1, c2: (P, 4, 2) -> puntos (P, 16, 2) y mascara (P, 16)."""
    p1 = c1.unsqueeze(2)
...
def _corners_inside(pts: torch.Tensor, box: torch.Tensor) -> torch.Tensor:
    """pts (P, 4, 2) dentro del rectangulo box (P, 4, 2) -> (P, 4)."""
    a = box[:, 0:1, :]
    ab = box[:, 1:2, :] - a
    ad = box[:, 3:4, :] - a
    ap = pts - a
...
def _convex_area(pts: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
    """Area del poligono convexo formado por los puntos validos. pts (P, K, 2), mask (P, K)."""
    p, k, _ = pts.shape
    ...
    # orden angular sin sort: rank_i = nº de puntos validos que van antes que i
...
def pairwise_rotated_iou(r1: torch.Tensor, r2: torch.Tensor) -> torch.Tensor:
    """IoU rotada exacta entre pares emparejados: (P, 5) x (P, 5) -> (P,)."""
    if r1.shape[0] == 0:
...
def rotated_iou(boxes1: torch.Tensor, boxes2: torch.Tensor) -> torch.Tensor:
    """IoU rotada exacta todos-contra-todos: (M, 5) x (N, 5) -> (M, N)."""
    boxes1 = boxes1.float()
    boxes2 = boxes2.float()
...
def batch_rotated_iou(boxes1: torch.Tensor, boxes2: torch.Tensor) -> torch.Tensor:
    """(B, M, 5) x (B, N, 5) -> (B, M, N). Equivale a `rotated_iou_similarity` de ppdet."""
