"""
PP-YOLOE-R in plain PyTorch (CPU / CUDA / MPS).

Faithful port of PaddleDetection release/2.8:
...
Submodule names match Paddle's, so the state_dict converts 1:1 (see checkpoint.py): only
bn._mean and bn._variance are renamed to bn.running_mean and bn.running_var.

Cross-framework equivalences worth knowing:
  - Paddle's swish is PyTorch's SiLU.
  - Paddle's hardsigmoid (slope 1/6, offset 0.5) is PyTorch's F.hardsigmoid.
  - F.interpolate(..., scale_factor=2) defaults to 'nearest' in both.
"""
...
class RepVggBlock(nn.Module):
    """Two branches, 3x3 and 1x1, summed with a learned alpha. Kept unfused."""
...
class PPYOLOERHead(nn.Module):
    def __init__(
        ...
        # DFL angle projection: a fixed 1x1 conv, not trainable, same as Paddle
    ...
    def bbox_decode(self, points, pred_dist, pred_angle, stride_tensor):
        """(B, L, 4) + (B, L, angle_max+1) -> rboxes (B, L, 5) in input pixels."""
...
class PPYOLOER(nn.Module):
    """backbone + neck + head. forward returns the raw head outputs."""
    ...
    def predict(self, images: torch.Tensor):
        """(B, 3, H, W) -> scores (B, C, L) and polys (B, L, 8), in input pixels."""
        self.eval()
...
def build_ppyoloe_r(num_classes: int = 1, size: str = "s") -> PPYOLOER:
    """Build PP-YOLOE-R with the official width/depth multipliers (s/m/l/x)."""
    mults = {"s": (0.33, 0.50), "m": (0.67, 0.75), "l": (1.0, 1.0), "x": (1.33, 1.25)}
    if size not in mults:
        raise ValueError(f"unknown size: {size} (use one of {sorted(mults)})")
