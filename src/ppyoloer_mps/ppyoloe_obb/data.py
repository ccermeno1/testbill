"""
Data: reading the COCO JSONs with polygons, inference preprocessing, and training
augmentation (mosaic, affine, HSV, flip, rotations), all in numpy/OpenCV.

Preprocessing matches PaddleDetection exactly:
  Decode      cv2.imdecode (BGR) -> RGB
  Resize      keep_ratio: scale = min(target_min/side_min, target_max/side_max),
              output size rounded as int(scale*side + 0.5)
  Normalize   divide by 255, then (x - mean) / std with ImageNet statistics
  Permute     HWC -> CHW
  PadBatch    zero padding up to a multiple of 32
"""
...
def load_image(path: str) -> np.ndarray:
    """Read an image as uint8 RGB. Handles HEIC when pillow_heif is installed."""
    if path.lower().endswith((".heic", ".heif")):
        if pillow_heif is None:
            raise RuntimeError(f"pillow-heif is required to read {path}")
        from PIL import Image, ImageOps

        with Image.open(path) as im:
            return np.asarray(ImageOps.exif_transpose(im).convert("RGB"))
    data = np.fromfile(path, dtype=np.uint8)  # copes with accented paths on Windows
...
def resize_keep_ratio(img: np.ndarray, target: int, interp: int = cv2.INTER_AREA):
    """Scale keeping the aspect ratio, same as ppdet's Resize(keep_ratio=True).
...
def preprocess_image(path_or_img, img_size: int = 640, interp: int = cv2.INTER_AREA):
    """Image -> tensor (1, 3, H, W) and scale_factor (2,) = (scale_y, scale_x)."""
...
@dataclass
class AugmentConfig:
    """Same augmentation used for the Paddle training runs."""

    img_size: int = 640
    mosaic_prob: float = 1.0
    mosaic_scale: float = 0.2      # random scale of +-20 % inside the mosaic
    mosaic_translate: float = 0.05
    affine_scale: float = 0.2      # used in the no-mosaic phase
    affine_translate: float = 0.05
    # [low, high, prob], with ppdet semantics: prob is the chance of SKIPPING the op
    ...
    fill_value: int = 114
    min_edge: float = 2.0        # drop boxes whose shorter side is under 2 px, like Poly2RBox
    rotate_fill_value: int = 0   # ppdet pads rotations with black (RandomRRotate fill_value=0)
...
def _clip_visible(polys: np.ndarray, w: int, h: int, min_visible: float) -> np.ndarray:
    """Mask of polygons whose visible area inside the canvas exceeds min_visible."""
    if len(polys) == 0:
...
def _min_edge_mask(polys: np.ndarray, min_edge: float) -> np.ndarray:
    """Mask of polygons whose shorter side, on the rotated rectangle, reaches min_edge.

    Same as ppdet's Poly2RBox(filter_threshold=2, filter_mode='edge'). Without it a degenerate
    box (w or h near zero) makes ProbIoU return NaN.
    """
    if len(polys) == 0:
        return np.zeros(0, dtype=bool)
...
def _random_distort(img: np.ndarray, cfg: "AugmentConfig", rng: random.Random) -> np.ndarray:
    """Colour jitter. Literal port of ppdet's RandomDistort(random_apply=False)
    (ppdet/data/transform/operators.py), which works through PIL ImageEnhance.

    Watch the semantics: each op is SKIPPED with probability prob (if uniform() < prob:
    return img), so prob is the chance of not applying it. Order is
    brightness -> [contrast] -> saturation -> hue -> [contrast], with a coin flip deciding
    whether contrast comes early or late.
    """
    ...
    def apply_hue(pil_img):
        ...
        # PIL's H channel is uint8 (0-255) and wraps around, same as in ppdet
...
def _rotate(img: np.ndarray, polys: np.ndarray, angle: float, fill: int):
    """Rotate with auto_bound: shrink the image just enough for the rotated content to fit
    the original canvas, so nothing is cut off. Mirrors RRotate(auto_bound=True) in
    ppdet/data/transform/rotated_operators.py, same centre and same angle sign."""
    h, w = img.shape[:2]
...
class Sample:
    image_path: str
    polys: np.ndarray  # (N, 8) in original-image pixels
    labels: np.ndarray  # (N,)
    image_id: int
    width: int
    height: int
...
class ObbDataset(Dataset):
    """Reads a COCO JSON whose annotations carry a 4-point polygon in `segmentation`."""
    ...
    def __getitem__(self, idx: int) -> dict[str, Any]:
        ...
        if len(polys):
            ...
            polys[:, 0::2] *= sx
            polys[:, 1::2] *= sy
            # ppdet's RResize does more than scale: it CLIPS the vertices to the canvas
            # (ppdet/data/transform/rotated_operators.py, RResize.apply_pts). Skip the clip
            # and the model learns to predict the part of the object it cannot see, which
            # inflates the boxes and tanks mAP75.
...
def collate(batch: Sequence[dict[str, Any]], stride: int = 32) -> dict[str, Any]:
    """Stack images padded to a multiple of `stride`, and pad the boxes to the longest run."""
