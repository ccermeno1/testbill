"""
PP-YOLOE-R loss: VariFocal for classification, ProbIoU for the rotated box, DFL for the angle.

Port of PPYOLOERHead.get_loss from ppdet/modeling/heads/ppyoloe_r_head.py.
"""
...
def weighted_bce(pred: torch.Tensor, target: torch.Tensor, weight: torch.Tensor) -> torch.Tensor:
    """Weighted BCE, written out so the gradient flows through `weight` too.

    PyTorch's F.binary_cross_entropy rejects a `weight` that requires grad, while Paddle's
    propagates through it, and PP-YOLOE-R's VariFocal builds the weight from the prediction
    itself. Checked numerically against Paddle. Logs are clamped at -100, same as PyTorch
    does internally.
    """
...
def df_loss(pred_dist: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
    """Distribution Focal Loss over the angle bin."""
...
class PPYOLOERLoss(nn.Module):
    ...
    def forward(self, head_outs, gt_rboxes, gt_labels, pad_gt_mask, decode_fn):
        """head_outs comes straight from PPYOLOERHead.forward_train.

        gt_rboxes (B, n, 5), gt_labels (B, n, 1), pad_gt_mask (B, n, 1).
        decode_fn is the head method turning (points, dist, angle, stride) into rboxes.
        """
