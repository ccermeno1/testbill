#!/usr/bin/env python
"""Pre-proceso, NMS y post-proceso fuera de la red, para un modelo ya exportado a ONNX.

El grafo exportado hace el forward y la decodificacion de cajas, porque ambas dependen solo de
las formas. Lo que NO se exporta es el filtro por score y el bucle de NMS, que dependen de los
valores. Esas ~40 lineas son las de este fichero, y son justo las que hay que replicar en el
cliente (movil, C++, etc.) para reproducir los resultados.

Ejecutalo para comprobar que los numeros coinciden con el modelo de PyTorch::

    python src/ppyoloer_mps/onnx_example.py <imagen> \
...
def preprocess(path: str, img_size: int = 640):
    """Imagen -> (1, 3, H, W) float32 RGB en 0..255, y la escala aplicada.

    La normalizacion vive DENTRO del grafo, asi que aqui solo se redimensiona y se rellena.
    Al reducir se usa INTER_AREA: la bilineal produce aliasing en fotos grandes y eso degrada
    de forma apreciable las metricas.
    """
    ...
    # relleno con ceros hasta multiplo de 32 (la red es totalmente convolucional)
...
def rbox2poly(boxes: np.ndarray) -> np.ndarray:
    """(n, 5) -> (n, 8). Angulo en radianes, eje y hacia abajo."""
    cx, cy, w, h, a = boxes.T
    cos, sin = np.cos(a), np.sin(a)
    # esquinas locales en el orden de PP-YOLOE-R, rotadas por [[cos, sin], [-sin, cos]]:
...
def rotated_iou(a: np.ndarray, b: np.ndarray) -> np.ndarray:
    """IoU rotada exacta (a: (n,5), b: (m,5)) -> (n, m), via recorte de poligonos convexos."""
...
def rotated_nms(boxes: np.ndarray, scores: np.ndarray, iou_thr: float) -> list[int]:
    """NMS codicioso sobre cajas rotadas. Devuelve indices, ordenados por score."""
...
def postprocess(scores: np.ndarray, boxes: np.ndarray, scale_x: float, scale_y: float,
    ...
    """Salida del grafo -> detecciones en coordenadas de la imagen ORIGINAL.

    scores (1, C, L), boxes (1, L, 5). Devuelve (rboxes, polys, scores, labels).
    """
    ...
    for cls in range(scores.shape[0]):
        ...
        if sel.size == 0:
            continue
        if sel.size > nms_pre:                     # nms_pre: acota el coste del NMS
    ...
    order = np.argsort(-sc)[:max_per_img]          # max_per_img: tope tras el NMS
    rb, sc, lb = rb[order], sc[order], lb[order]
    # volver a la imagen original: deshacer la escala del resize (el relleno va arriba-izquierda,
    # asi que no hay desplazamiento que corregir)
...
def main() -> None:
    ...
    ap.add_argument("--checkpoint", default=None, help="checkpoint .pt para contrastar los resultados")
    ...
    print(f"ONNX: {len(sc)} detecciones (conf >= {args.conf}, NMS IoU {args.nms_iou})")
    ...
    if args.checkpoint:
        ...
        print(f"PyTorch: {len(det['scores'])} detecciones")
        if len(rb_t) == len(rb) and len(rb):
            print(f"  diferencia maxima en las cajas: {np.abs(np.sort(rb_t, 0) - np.sort(rb, 0)).max():.4f} px")
            print(f"  diferencia maxima en los scores: {np.abs(np.sort(det['scores'].numpy()) - np.sort(sc)).max():.6f}")
        elif len(rb_t) != len(rb):
            print("  AVISO: distinto numero de detecciones entre ONNX y PyTorch")
