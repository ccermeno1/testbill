#!/usr/bin/env python
"""
Evaluate a checkpoint from this repo on one or more splits (COCO JSON with polygons).
...
def main() -> None:
    ...
    ap.add_argument("--data", required=True, help="dataset folder (images/ + annotations/)")
    ap.add_argument("--split", nargs="+", default=["valid", "test"])
    ap.add_argument("--img-size", type=int, default=640)
    ap.add_argument("--batch", type=int, default=4)
    ap.add_argument("--workers", type=int, default=0)
    ap.add_argument("--conf", type=float, default=0.5, help="threshold for P/R/F1")
    ap.add_argument("--score-threshold", type=float, default=0.05, help="minimum score kept for the AP curve")
    ap.add_argument("--nms-iou", type=float, default=0.1)
    ap.add_argument("--device", default="auto")
    ap.add_argument("--out", default=None, help="folder to write the metric JSONs into")
    args = ap.parse_args()
    ...
    print(f"device: {device} | weights: {args.weights} | classes: {meta.get('classes')}")
    print(HEADER)
