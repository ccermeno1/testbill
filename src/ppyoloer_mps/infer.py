#!/usr/bin/env python
"""
Predict oriented boxes on single images or a folder, and write the results out.
...
Output:
  <out-dir>/<stem>.txt            class score x1 y1 ... x4 y4, in original-image pixels
  <out-dir>/yolo_obb/<stem>.txt   normalised YOLOv8-OBB format
  <out-dir>/predictions.json      everything in one file
  <out-dir>/vis/<stem>.jpg        only with --vis
"""
...
def main() -> None:
    ...
    ap.add_argument("--images", nargs="+", required=True, help="images or folders")
    ...
    print(f"device: {device} | {len(images)} images | conf {args.conf}")
    ...
    print(f"{total} boxes (score >= {args.conf}) over {len(images)} images -> {os.path.abspath(args.out_dir)}")
