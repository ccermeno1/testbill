...
def main():
    ...
    p.add_argument('checkpoint', help='checkpoint filename or path')
    p.add_argument('images', nargs='?', default='data/external_dataset/images', help='image file or folder')
    p.add_argument('--out', default='infer')
    ...
    args.images = str(Path(args.images) if Path(args.images).exists() else ROOT / args.images)
    out = Path(args.out)
    experiment_dir = Path(args.checkpoint).parent
    args.out = str(out if out.is_absolute() else experiment_dir / out)
