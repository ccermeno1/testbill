...
# OpenCV's thread pool can conflict with DataLoader workers on macOS.
cv2.setNumThreads(0)
...
def resize_image(img: np.ndarray, width: int, height: int) -> np.ndarray:
    """Resize BGR data with antialiasing when shrinking."""
    ...
    if RESIZE_BACKEND != 'pil':
        raise ValueError(f'unknown resize backend: {RESIZE_BACKEND!r}')
    from PIL import Image
    resampling = getattr(Image, 'Resampling', Image).BILINEAR
    return np.ascontiguousarray(np.asarray(Image.fromarray(img).resize((width, height), resampling)))
...
class StrongAug:
    ...
    Mosaic and mixup use caches of recent samples, so each item is decoded once
    instead of recursively running the complete augmentation pipeline."""
...
class YoloObbDataset(Dataset):
    ...
    def __init__(self, root: str, split: str, img_size: int = 640, train: bool = False,
        ...
        elif osp.isdir(osp.join(root, 'images')):
            # Also accept a flat evaluation folder: <root>/images + labels.
            for img in sorted(glob.glob(osp.join(root, 'images', '*'))):
                if not img.lower().endswith(IMG_EXTS):
                    continue
                stem = osp.splitext(osp.basename(img))[0]
                lbl = osp.join(root, 'labels', stem + '.txt')
                self.items.append(dict(
                    id=stem, img=img, label=lbl if osp.exists(lbl) else None, split=split
                ))
        else:
    ...
    def _mosaic(self, sample):
        """mmdet CachedMosaic: the current sample plus three cached samples."""
        s = self.img_size
        ...
        for loc, (img, boxes, labels) in zip(
                ('top_left', 'top_right', 'bottom_left', 'bottom_right'), [sample] + others):
            h, w = img.shape[:2]
            if loc == 'top_left':
