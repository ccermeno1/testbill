...
Hub slug **`rotated_retinanet_dota_le90_3x_hbb`**. Official DOTA v1.0 Task 1 **70.70%** (AP75 43.34, COCO mAP 41.59). Deploy `production.score_threshold` **0.35** (eval-val F1 0.40 − 0.05). **Circum-HBB matching** (`use_hbb_for_matching: true`); +2.83 Task 1 / +3.26 AP75 vs Hub 1× HBB **67.87%**. Default Hub 1× / 3× are now OBB (`rotated_retinanet_dota_le90_1x` / `rotated_retinanet_dota_le90_3x`). The mAP50 below is leaky eval-val (val tiles are in train), not the published number.
