...
Hub slug **`rotated_retinanet_dota_le90_1x_hbb`**. Official DOTA v1.0 Task 1 **67.87%** (AP75 40.08, COCO mAP 38.91). Deploy `production.score_threshold` **0.35** (eval-val F1 0.40 − 0.05). **Circum-HBB matching** (`use_hbb_for_matching: true`); ahead of MMRotate HBB (**64.55%**). Default Hub 1× is now OBB (`rotated_retinanet_dota_le90_1x`). The mAP50 below is leaky eval-val (val tiles are in train), not the published number.
